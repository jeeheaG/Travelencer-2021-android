    ...
    override fun onCreate(savedInstanceState: Bundle?) {
       ...
       //val placeList = arrayListOf<ModelCasePlaceCard>()
        ...
        placeList = arrayListOf(
            ...
            ModelCasePlaceCard(dummyImageUrl, "무슨산", "경기도 수원시", dummyContentId),
            ModelCasePlaceCard(dummyImageUrl, "관광지", "경기도 수원시", dummyContentId),
            ModelCasePlaceCard(dummyImageUrl, "뭐라고", "경기도 수원시", dummyContentId),
            ModelCasePlaceCard(dummyImageUrl, "광교산", "경기도 수원시", dummyContentId),
            ModelCasePlaceCard(dummyImageUrl, "왕갈비", "경기도 수원시", dummyContentId),
        ...
        binding.rvBookmarkList.adapter = placeAdapter
        firestore.collection("bookmarkT").whereEqualTo("uid",uid).get()
            .addOnSuccessListener { docs->
                for(doc in docs){
                    val placeId = doc.data.get("placeId").toString()
                    callTourDetailCommon(placeId)
                }
            }
            ...
            .enqueue(object : retrofit2.Callback<ModelTourApiDetailCommon> {
                ...
                override fun onResponse(
                    ...
                    if(body!=null){
                        ...
                        if(firstImage != null){
                            placeList.add(ModelCasePlaceCard(firstImage,item.title,item.addr1 ?: "-",contentId))
                        }
