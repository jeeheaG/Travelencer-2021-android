...
class PostWritePlaceSearchActivity : AppCompatActivity() {
    ...
    private lateinit var mAdapter: PostWritePlaceSearchAdapter
...
/*        //장소 검색 더미데이터
        // TODO : url로 바꿈
        //val placeList = arrayListOf<ModelCasePlaceCard>()
        ...
        val dummyContentId = "2360786"
        val placeList = arrayListOf(
                ModelCasePlaceCard(dummyImageUrl, "해우재", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "수원 화성", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "무슨산 공원", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "수원 관광지", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "뭐라고갈비", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "광교산", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "수원 왕갈비", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "무슨호수", "경기도 수원시", dummyContentId),
                ModelCasePlaceCard(dummyImageUrl, "저기호수공원", "경기도 수원시", dummyContentId)
        )*/
        ...
        val postWritePlaceSearchLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){ result ->
            if(result.resultCode == RESULT_CODE_WRITE){
                val intent = Intent(this, PostWriteActivity::class.java)
                val data = result.data
                intent.putExtra("placeId", data?.getStringExtra("placeId"))
        ...
        binding.btnPlaceSearch.setOnClickListener {
            ...
            val keyword = binding.etPlaceSearchKeyword.text.toString()
            mAdapter = PostWritePlaceSearchAdapter(placeList, this, postWritePlaceSearchLauncher)
            binding.rvPlaceSearch.adapter = mAdapter
            callTourKeyword(keyword)
    ...
    private fun callTourKeyword(keyword: String) { //, areaCode: String, sigunguCode: String
        ...
        // api 요청보냄
        tourApi.getTourKeywordData(keyword = keyword, null, null) //, area = areaCode, sigungu = sigunguCode
            .enqueue(object : retrofit2.Callback<ModelTourApiKeyword> {
                ...
                override fun onFailure(call: Call<ModelTourApiKeyword>, t: Throwable) {
                    ...
                    Toast.makeText(applicationContext, "검색 결과가 없거나 1개 뿐입니다. 다른 검색어로 검색해 주세요", Toast.LENGTH_LONG).show()
                    Log.e("실패 로그", "실패..")
                    t.printStackTrace()
                }
            })
